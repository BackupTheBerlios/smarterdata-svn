<?php
	...
	*/
	function smarttemplate_extension_options_multi ( $param, $values, $default = '_DEFAULT_' )
	{
		$output  =  "";
		if (is_array($param))
		{
			...
			foreach ($values as $key => $value)
			{
				...
				{
					$output  .=  '<option' . (($value == $default) ? '  selected' : '') . '>' . $param[$key] . '['.$value.']</option>';
				}
				else
				{
					$output  .=  '<option value="' . $key . '"' . (($key == $default) ? '  selected' : '') . '>' . $param[$key] . '['.$value.']</option>';
				}
			}
		}
		return $output;
