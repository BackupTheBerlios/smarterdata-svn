...
class SmarterTemplate
{
	...
	public function __construct($template_filename)
	{
		...
		$this->allowIncompatibleFeatures= (isset ($GLOBALS['_CONFIG']['allowIncompatibleFeatures'])) ? $GLOBALS['_CONFIG']['allowIncompatibleFeatures'] : false;
		...
		if ($this->compatibility === true && $this->allowIncompatibleFeatures !== true)
		{
			$this->lang= false;
		}
		else
	...
	private function setFeatures()
	{
		 ...
		 */
		if (isset ($GLOBALS['_CONFIG']['enableXML']) && $GLOBALS['_CONFIG']['enableXML'] === false)
		{
			$this->enableXML= false;
		}
		if ($this->compatibility === true)
			$this->enableXML= true;
		if (isset ($GLOBALS['_CONFIG']['enableExtension']) && $GLOBALS['_CONFIG']['enableExtension'] === false)
		{
		...
		if ($this->compatibility === true)
			$this->enablePHPExecution= true;
		if (isset ($GLOBALS['_CONFIG']['sqlDSN']) && isset ($GLOBALS['_CONFIG']['sqlUsername']) && isset ($GLOBALS['_CONFIG']['sqlPassword']))
		{
			$this->enableSql= true;
		...
		{
			/* IDE-Workaround */
			$require= "class.SmarterCache.php";
	...
	{
		if (isset ($GLOBALS['_CONFIG']['doDebug']) && $GLOBALS['_CONFIG']['doDebug'] === true)
		{
			$this->doDebug= true;
		}
		else
	...
	private function setPaths()
	{
		...
		$this->templateDirectoryEncoded= preg_replace('/[:\/.\\\\]/', '_', $this->templateDirectory);
		// Compiled
		...
		{
			$this->compiledDirectory= $this->thisdir . "/internal/compiled";
		}
		// Extensions
		...
		else
		{
			if ($this->compatibility === true)
			{
				$this->extensionDirectory= $this->thisdir . "/internal/smarttemplate_extensions";
			}
			else
			{
				$this->extensionDirectory= $this->thisdir . "/internal/extensions";
			}
		}
	...
	{
		if (isset ($GLOBALS['_CONFIG']['cache_lifetime']) && is_numeric($GLOBALS['_CONFIG']['cache_lifetime']))
		{
	...
	private function compile()
	{
		...
		if ($this->useCache === true)
		{
			if (isset ($GLOBALS['_CONFIG']['mtime']) && $GLOBALS['_CONFIG']['mtime'] === true)
			{
		...
		if ($this->useCache === false || $cachedDate === false)
		{
			...
			if ($this->enablePHPExecution === false)
				$site= $this->replacePHPCode($site);
	...
	private function getTemplate()
	{
		...
		if (isset ($GLOBALS['_CONFIG']['autoLink']) && $GLOBALS['_CONFIG']['autoLink'] == true)
		{
			$return= $this->autoAddLanguage($return);
		}
		if ($this->doDebug === true)
			$return= str_replace('CONTENT_HERE', $return, $debug);
		return $return;
	}
	private function addDebug()
	{
		...
		$replacements= str_replace("    ", "&nbsp;&nbsp;", htmlentities($replacements));
		$return= "
										<table style=\"text-align: left; width: 100%;\" border=\"1\" cellpadding=\"2\" cellspacing=\"2\">
											...
											<tr>
												<td>" . highlight_string(str_replace("\t", "  ", $code), 1) . "</td>
											</tr>
										</table>";
		$return .= ob_get_flush();
		return $return;
	...
	private function replaceExtension($site, $currentLoop)
	{
		...
		if (preg_match_all($pattern, $site, $result))
		{
			foreach ($result[1] as $key => $extension)
			{
				...
				$site= str_replace($result[0][$key], "<?php echo " . $functionname . $extension . " ( $arguments ); ?>", $site);
			}
		}
		return $site;
	}
	...
	private function convertReplacement($replacement, $currentLoop)
	{
		...
		if (substr($replacement, 0, 4) == "top." || substr($replacement, 0, 4) == "TOP.")
		{
			$currentLoop= 0;
